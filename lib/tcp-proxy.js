...
class TCPProxy {
  ...
  async init() {
    await this.vmProxy.impl.reloadVmInfo();
    console.log(`Setting up tcp proxy ${this.host}:${this.port} -> ${this.vmProxy.impl.ipAddress}:${this.remotePort}`)
  ...
  async _onClientConnection(client) {
    let whitelisted = await this.vmProxy.impl.whitelist(client.address().address);
    if( !whitelisted ) {
      return console.log('Ignoring packet from: '+client.address().address);
    }    

    let connection = new TCPConnection(client, this.vmProxy.impl.ipAddress, this.remotePort, this.vmProxy);
